...
public class DefaultDestinations {
	...
	public DefaultDestinations() {
		...
		for(IndicatorItem item : Api.analyst.getIndicatorsById("jobs_edu")) {
			if(!destinations.containsKey(item.geoId))
				destinations.put(item.geoId, item.sample);
		}
		
		for(IndicatorItem item : Api.analyst.getIndicatorsById("workforce_type")) {
			if(!destinations.containsKey(item.geoId))
				destinations.put(item.geoId, item.sample);
		}
		
		for(IndicatorItem item : Api.analyst.getIndicatorsById("workforce_edu")) {
			if(!destinations.containsKey(item.geoId))
				destinations.put(item.geoId, item.sample);
		}
